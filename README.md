...
The **entity-resolution model** (P6.24) is trained and integrated as an advisory signal. On a frozen, entity-disjoint held-out partition of 2,615 real record pairs it recovers **317 of 356** true same-entity pairs where the deterministic resolver recovers **158** — closing name-containment cases completely (0% → 100%) and making real progress on script variants, partial overlaps and divergent names — at a cost of one additional false merge (2 → 3 of 2,259 negatives). It reads **no identifier**, because every label in the project is derived from identifier agreement and an identifier feature would be the answer rather than evidence; the leakage gate enforces that in the type system, in the data and by inspection. **It does not merge anything.** `src/lib/resolution/` is unchanged and remains the sole authority on identity; the model emits an `algorithmic_signal` carrying its score, threshold, model version, the deterministic verdict beside it, and all 25 feature values, so a suggestion is always shown with its evidence and never as a fact. Its three false merges are all group-and-member pairs whose governing policy (P6.21.2) is still undecided, and that is recorded as the model's principal bottleneck rather than tuned away (`docs/evaluation/ml-model-card.md`, `docs/architecture/ml-integration.md`).
...
| Used for | The demonstration flow | Resolver evaluation and, at P6.24, model training |
