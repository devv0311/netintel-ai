...
Measured on a **final frozen test** — 5,257 real record pairs over 963 subjects that appear in no partition of any earlier dataset, collected after all feature work was finished and scored once — it recovers **682 of 892** true same-entity pairs where the deterministic resolver recovers **434**, closing containment (0% → 85.9%), script variants (0% → 87.6%) and divergent names (0% → 49.4%). Both halves of the result are reported: over the 244 curated hard negatives it suggests a merge for **41** where the resolver merges **16**, and **every one of its 46 false merges is a corporate-family pair** — `BARCLAYS PLC` against `BARCLAYS BANK PLC`, `ROLLS-ROYCE HOLDINGS PLC` against `ROLLS-ROYCE PLC`. That is the still-undecided P6.21.2 policy question with a number attached, and it is why the score stays advisory rather than being promoted (`docs/evaluation/ml-model-card.md`, `docs/architecture/ml-integration.md`).
