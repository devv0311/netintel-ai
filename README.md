...
│   └── lib/                     # ai/, analytics/ (P5.6 topology analytics pipeline), corroboration/
│                                   (P5.7 spatial/temporal corroboration pipeline), corpus/
...
Ingestion, extraction, resolution, graph synthesis, topology analytics, and
spatial/temporal corroboration are fully local and deterministic (one SQLite
file, one JSON corpus, no Anthropic call, no Docker). Details:
...
items agree, and is otherwise an Algorithmic Signal.
