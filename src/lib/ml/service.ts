...
const DISCLAIMER =
  ...
  "Two limitations, both measured on a frozen test rather than asserted, and one caveat about the measurement itself. " +
  "A HIGH score can be a corporate family, not one entity: on the P6.28 frozen test this model suggested a merge " +
  "for 5.42% of genuine name collisions between DIFFERENT legal entities against the deterministic resolver's " +
  "0.24%, and 286 of its 289 false merges were corporate-family pairs — a company against a same-named affiliate " +
  "carrying a different registration. Treat a high score between two similar names in one corporate group as " +
  "unproven; the relationship policy that would govern such pairs (P6.21.2) is not " +
  "yet decided. " +
  "A LOW score across a border is close to no evidence at all: where the two publishers state DIFFERENT " +
  "countries this model recovers 16.7% of genuine same-entity pairs where the deterministic resolver recovers " +
  "63.3%. A company incorporated offshore and operating onshore is one company, and this model usually misses " +
  "it. Do not read a low score between records in different countries as evidence that they differ. " +
  "And that cross-border number rests on 30 pairs. The frozen test this model was selected on is broad in " +
  "volume and narrow in jurisdiction — 25 countries, no non-Latin script, and two thirds of its true pairs are " +
  "cases where one publisher's name is contained in the other's. Its aggregate recall of 94.1% is therefore a " +
  "measurement of an easier distribution than the earlier tests posed, and is not comparable with them.";
