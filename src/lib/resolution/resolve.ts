...
const IDENTITY_RELATIONSHIP_TYPES = ["has_phone", "has_account", "has_vehicle"] as const;
...
export function resolveEntities(
  ...
  // --- Phase 2: Tier-A clustering of person mentions via shared identifiers ---

  const personMentions = entityMentions.filter((r) => str(r.data, "mentionKind") === "person");
  ...
  for (const [root, memberIds] of [...clustersByRoot.entries()].sort(([a], [b]) =>
    ...
    const entityId = makeContentId("entity", ["person", sortedMemberIds.join(",")]);
  ...
  // --- Phase 3: Tier-B — resolve every remaining (identifier-less) person mention ---
  ...
  for (const mention of [...personMentions].sort((a, b) => (a.id < b.id ? -1 : 1))) {
    ...
    if (candidates.length >= 2) {
      const standaloneId = makeContentId("entity", ["person", mention.id]);
      entities.push({
        id: standaloneId,
        investigationId,
        kind: "person",
