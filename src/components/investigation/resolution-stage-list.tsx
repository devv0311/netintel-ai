...
/**
 * Renders the eight resolution stages and their real, server-reported
 * status. There is no timed animation — a stage only advances when the
 * stream says it did. Mirrors investigation/extraction-stage-list.tsx.
 */
export function ResolutionStageList({
  stages,
  persistProgress,
}: {
  stages: StageReport[];
  ...
  return (
    <ol className="flex flex-col gap-1.5" aria-label="Resolution stages">
      {RESOLUTION_STAGES.map((stage) => {
        ...
        return (
          <li
            key={stage}
            data-testid={`resolution-stage-${stage}`}
            data-status={status}
