...
/**
 * Renders the seven extraction stages and their real, server-reported
 * status. There is no timed animation — a stage only advances when the
 * stream says it did. Mirrors investigation/stage-list.tsx (ingestion).
 */
export function ExtractionStageList({
  stages,
  persistProgress,
}: {
  stages: StageReport[];
  ...
  return (
    <ol className="flex flex-col gap-1.5" aria-label="Extraction stages">
      {EXTRACTION_STAGES.map((stage) => {
        ...
        return (
          <li
            key={stage}
            data-testid={`extraction-stage-${stage}`}
            data-status={status}
