...
import { CORROBORATION_STAGES, CORROBORATION_STAGE_LABELS, type StageReport } from "@/lib/corroboration/types";

/**
 * Renders the ten spatial/temporal corroboration stages and their real,
 * server-reported status. There is no timed animation — a stage only
 * advances when the stream says it did. Mirrors
 * investigation/analytics-stage-list.tsx.
 */
export function CorroborationStageList({
  stages,
  persistProgress,
}: {
  stages: StageReport[];
  ...
  return (
    <ol className="flex flex-col gap-1.5" aria-label="Corroboration stages">
      {CORROBORATION_STAGES.map((stage) => {
        ...
        return (
          <li
            key={stage}
            data-testid={`corroboration-stage-${stage}`}
            data-status={status}
          ...
          >
            ...
            <div className="flex min-w-0 flex-col">
              ...
              >
                {CORROBORATION_STAGE_LABELS[stage]}
              </span>
