...
import { ANALYTICS_STAGES, ANALYTICS_STAGE_LABELS, type StageReport } from "@/lib/analytics/types";

/**
 * Renders the ten topology-analytics stages and their real,
 * server-reported status. There is no timed animation — a stage only
 * advances when the stream says it did. Mirrors
 * investigation/graph-stage-list.tsx.
 */
export function AnalyticsStageList({
  stages,
  persistProgress,
}: {
  stages: StageReport[];
  ...
  return (
    <ol className="flex flex-col gap-1.5" aria-label="Analytics stages">
      {ANALYTICS_STAGES.map((stage) => {
        ...
        return (
          <li
            key={stage}
            data-testid={`analytics-stage-${stage}`}
            data-status={status}
