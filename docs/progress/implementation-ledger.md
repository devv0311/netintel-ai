...
| P4.2 | Domain/data foundation — 12 Zod-validated domain types (`src/lib/domain/`: investigation, evidence source/item, extracted record, entity, alias, location, temporal interval, communication event, financial transaction, relationship, analytical signal, AI inference, investigative lead) with confidence kept separate from the 5-value evidence classification; deterministic content-addressed + opaque ID generation (`ids.ts`); a validated repository layer (`src/lib/db/repository.ts`) over a 12-table migrated SQLite schema (`schema.ts`, superseding the P4.1 placeholder); a synthetic-fixture + isolated ground-truth-fixture loading boundary (`src/lib/fixtures/`, `evidence/*/fixtures/foundation-smoke*`) — not the full Operation DarkNet Delhi dataset | Completed | Claude Code (AI) | Pending (this commit) | N/A — backend/domain foundation only, no new UI surface | 35/35 Vitest unit tests pass (7 files: ids, domain-validation, provenance, db, fixtures, env, graph); 1/1 Playwright E2E test still passes unmodified; DB smoke test (empty→migrate→insert→select→verify provenance→re-migrate) passes; `tsc --noEmit` clean; `eslint .` clean; `next build` succeeds; no dependency changes | No — foundation only, no pipeline stage implemented |
